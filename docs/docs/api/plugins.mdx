...
立即启动一个已配置的 cron job。`job_name` 是经过 URL 编码的 `args.jobs[].name`；手动触发使用与定时触发相同的 executor 列表和重叠保护。对于 `interval` 任务，下一次运行时间会从本次手动触发时刻重新计算；使用 cron 表达式的任务仍保持原计划。

返回：

* `202 Accepted`
  * 任务已在后台启动，响应中的 `status` 为 `started`、`trigger` 为 `manual`。
* `404 Not Found`
...
WebUI 会在已应用的 cron 插件详情中为每个任务显示“手动运行”按钮。编辑未保存配置时按钮隐藏，避免用尚未应用的任务名称触发旧 runtime。
