...
保存时，浏览器编译器迁移并规范化意图、检查构建能力与引用、显示重复/被覆盖/不可达规则并生成候选配置；通用 `/config/validate` 对原生 YAML 和真实相对路径执行最终预检。确认后才通过可恢复的通用事务应用。schema v4 会迁移为 v5；旧的 ECS 和 IP 优选占位值会给出复核警告，不会静默扩大路由范围。
