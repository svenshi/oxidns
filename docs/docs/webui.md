...
- 标准模式由浏览器内 TypeScript 编译器生成 Plan、诊断、配置所有权和插件差异；未托管或被修改的配置必须显式确认接管。
- 确认后，原生 DNS YAML 通过通用配置事务应用。失败会恢复旧文件和旧运行时；工作区 JSON 保存失败不回滚 DNS。
- 标准模式提供多上游组、路径引用与删除保护、独立过滤订阅、本地域名策略、查询日志导出、按路径缓存管理和后端配置历史。
- schema v5 还提供六类语义数据、国内/远程路径、三种未知域名模式、响应校验回退、ECS、双栈和 DNSSEC 安全的 IP 优选；规则数据页显示缺失、过期、下载与 Provider 装载状态。
- “本地域名策略”只使用 OxiDNS 原生插件；“运行与恢复”中的历史恢复只取回 YAML 预览，再经过差异预览和通用事务，不直接覆盖配置。
- 只切换 WebUI 视图不会重写 DNS YAML。模式边界与通用接口见[标准模式与后端边界](api/standard-mode.mdx)。
- 上游防泄漏只约束到达 OxiDNS 的查询，完整边界和配置语义见[标准模式智能分流](standard-mode-smart-routing.md)。
