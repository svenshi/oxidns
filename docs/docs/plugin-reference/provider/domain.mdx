...
- `max_entries`：可选的总条目硬上限；达到上限时整批拒绝新条目，不做部分写入。
- `entry_ttl_seconds`：可选的自动学习条目有效期；只淘汰 `learned` 来源，API 手工修正规则不会过期。
- `cleanup_interval_seconds`：过期扫描周期，默认 `600` 秒，不在查询热路径执行。
- `metadata_path`：来源与学习时间的原子 JSON 侧车文件；启用 TTL 时必填。
...
| `GET` | `/status` | 返回 learned/manual 数量、容量、过期/拒绝计数、最后成功时间和错误。 |
