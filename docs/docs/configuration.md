...
因为替换发生在 YAML 文本层，如果环境变量值包含 `*`、`&`、`:`、`#`、换行等 YAML 特殊字符，请把占位符放在引号里：`"${PW}"`（双引号）或 `'${PW}'`（单引号）。OxiDNS 会按 YAML 引号规则对替换后的值做转义，使值中的 `'`、`"`、`\` 与换行也能安全穿过 YAML 解析。占位符未加引号时，替换为字面量，值中的 YAML 特殊字符会按 YAML 语法解释（例如以 `*` 开头会被当作锚点引用）。`include` 路径同样支持占位符，例如：
