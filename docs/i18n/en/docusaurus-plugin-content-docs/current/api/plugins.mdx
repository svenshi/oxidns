...
Starts one configured cron job immediately. `job_name` is the URL-encoded `args.jobs[].name`. A manual trigger uses the same executor list and overlap protection as scheduled triggers, but does not change the next scheduled run.
