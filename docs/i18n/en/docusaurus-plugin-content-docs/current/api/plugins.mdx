...
Starts one configured cron job immediately. `job_name` is the URL-encoded `args.jobs[].name`. A manual trigger uses the same executor list and overlap protection as scheduled triggers. For an `interval` job, the next run is recalculated from the manual trigger time; cron-expression jobs keep their existing schedule.

Responses:

* `202 Accepted`
  * The job started in the background. The response contains `status: "started"` and `trigger: "manual"`.
* `404 Not Found`
...
* `503 Service Unavailable`
  * The cron scheduler is stopping or unavailable.

The WebUI shows **Run now** for every job in the detail view of an applied cron plugin. The action is hidden while editing unsaved configuration so a name that has not been applied cannot target the old runtime.
