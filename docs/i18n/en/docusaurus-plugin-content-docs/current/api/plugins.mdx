...
The WebUI shows **Run now** for every cron job only when the displayed config version is active. The action is hidden while editing or after saving an unapplied version so the displayed configuration cannot target an older runtime.
