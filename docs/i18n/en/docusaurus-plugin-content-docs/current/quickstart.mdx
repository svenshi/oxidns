...
Linux x86_64 / ARM64 musl also get `minimal` and `standard` slim archives. `minimal` only includes `oxidns`, `config.yaml`, and `LICENSE`; `standard` also includes WebUI static files, query_recorder, and the upgrade capability.
