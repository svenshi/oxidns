...
- The management API and WebUI can start an individual job manually with the same overlap protection as scheduled triggers. An `interval` job recalculates its next run from the manual trigger time, while a cron-expression job keeps its existing schedule.
