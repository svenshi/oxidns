...
- Purpose: Starts with new learning paused while existing dynamic rules continue to match.

Runtime controls are `GET /api/plugins/<tag>/status`, `POST /pause`, and `POST /resume`. Pausing only disables the learning side effect; it never stops the downstream DNS chain.
...
- tag: query_recorder_main
  ...
  args:
    ...
    # Optional: record only when the context has any listed mark
    include_marks: [100]
    # Optional: skip when the context has any listed mark; exclusion wins
    exclude_marks: [101]
```
...
- Purpose: A non-empty `include_marks` list records only when the context contains at least one listed mark. Any `exclude_marks` hit always skips the row. Empty lists preserve the original record-all behavior.
...
- Mark filters are evaluated after `next` returns, allowing a downstream path to decide persistence while an entry recorder still captures the complete routing and fallback event chain.
