...
- tag: learned_allow
  ...
  args:
    ...
    metadata_path: "/etc/oxidns/learned-allow.meta.json"
```
...
- `max_entries`: optional hard total-entry limit; a new batch is rejected atomically at capacity.
- `entry_ttl_seconds`: optional age limit for learned entries; manual API corrections do not expire.
- `cleanup_interval_seconds`: bounded expiration scan interval, default `600` seconds, outside the query hot path.
- `metadata_path`: atomically replaced JSON sidecar for origin and learned timestamps; required when TTL is enabled.
...
| `GET` | `/status` | Returns learned/manual counts, limits, expiration/rejection counters, and last success/error. |
